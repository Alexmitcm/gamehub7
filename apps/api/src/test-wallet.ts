...
async function testWallet() {
  ...
  try {
    ...
    if (isPremium) {
      ...
      console.log(`✅ USDT Balance: ${usdtBalance} (${Number(usdtBalance) / 1e18} USDT)`);
      
      const hasSufficientBalance = await BlockchainService.hasSufficientUsdtBalance(TEST_WALLET);
      console.log(`✅ Sufficient USDT (200+): ${hasSufficientBalance ? "YES" : "NO"}`);
      
      const referralReward = await BlockchainService.getReferralReward(TEST_WALLET);
      console.log(`✅ Referral Reward: ${referralReward} (${Number(referralReward) / 1e18} USDT)`);
      
      const gameRewards = await BlockchainService.getGameVaultRewards(TEST_WALLET);
      console.log(`✅ Game Vault Rewards:`);
      console.log(`   🎮 Balanced: ${gameRewards.balanced} (${Number(gameRewards.balanced) / 1e18} USDT)`);
      console.log(`   🎮 Unbalanced: ${gameRewards.unbalanced} (${Number(gameRewards.unbalanced) / 1e18} USDT)`);
      ...
      console.log(`   💰 Referral Reward: ${profileStats.referralReward} (${Number(profileStats.referralReward) / 1e18} USDT)`);
      console.log(`   🎮 Balanced Reward: ${profileStats.balancedReward} (${Number(profileStats.balancedReward) / 1e18} USDT)`);
      console.log(`   🎮 Unbalanced Reward: ${profileStats.unbalancedReward} (${Number(profileStats.unbalancedReward) / 1e18} USDT)`);
    }
    ...
    if (userProfile) {
      ...
      console.log(`   👤 Display Name: ${userProfile.displayName || "Not set"}`);
    ...
    const availableProfiles = await UserService.getAvailableProfiles(TEST_WALLET);
    ...
    if (availableProfiles.linkedProfile) {
      console.log(`   🔗 Already Linked: ${availableProfiles.linkedProfile.handle}`);
    }
    ...
    const premiumStatus = await PremiumService.getUserPremiumStatus(TEST_WALLET);
    console.log(`✅ Premium Status: ${premiumStatus.userStatus}`);
    
    if (premiumStatus.linkedProfile) {
      console.log(`   🔗 Linked Profile: ${premiumStatus.linkedProfile.handle}`);
    ...
    const profileStatsFromPremium = await PremiumService.getProfileStats(TEST_WALLET);
    console.log(`✅ Profile Stats from Premium Service:`);
    console.log(`   💰 Referral Reward: ${profileStatsFromPremium.referralReward} (${Number(profileStatsFromPremium.referralReward) / 1e18} USDT)`);
    console.log(`   🎮 Balanced Reward: ${profileStatsFromPremium.balancedReward} (${Number(profileStatsFromPremium.balancedReward) / 1e18} USDT)`);
    console.log(`   🎮 Unbalanced Reward: ${profileStatsFromPremium.unbalancedReward} (${Number(profileStatsFromPremium.unbalancedReward) / 1e18} USDT)`);
    ...
    if (userRewards.length > 0) {
      console.log(`   📊 Pending: ${userRewards.filter(r => r.status === "Pending").length}`);
      console.log(`   ✅ Claimed: ${userRewards.filter(r => r.status === "Claimed").length}`);
      console.log(`   ❌ Failed: ${userRewards.filter(r => r.status === "Failed").length}`);
      console.log(`   ⏰ Expired: ${userRewards.filter(r => r.status === "Expired").length}`);
    ...
    if (userQuests.length > 0) {
      console.log(`   🎯 Active: ${userQuests.filter(q => q.status === "Active").length}`);
      console.log(`   ✅ Completed: ${userQuests.filter(q => q.status === "Completed").length}`);
      console.log(`   ⏰ Expired: ${userQuests.filter(q => q.status === "Expired").length}`);
      console.log(`   ❌ Failed: ${userQuests.filter(q => q.status === "Failed").length}`);
    }
    
    // Test user notifications
    const userNotifications = await UserService.getUserNotifications(TEST_WALLET, false, 10);
    console.log(`✅ User Notifications: ${userNotifications.length} unread notifications`);
    
    if (userNotifications.length > 0) {
      const notificationTypes = [...new Set(userNotifications.map(n => n.type))];
    ...
    console.log(`   🎮 Balanced Game Vault: ${contractAddresses.balancedGameVault}`);
    console.log(`   🎮 Unbalanced Game Vault: ${contractAddresses.unbalancedGameVault}`);
    ...
    console.log(`   ⭐ Premium Status: ${summary.isPremium ? "PREMIUM" : "STANDARD"}`);
    console.log(`   👤 User Profile: ${summary.hasUserProfile ? "EXISTS" : "NOT FOUND"}`);
    console.log(`   🔗 Linked Profile: ${summary.hasLinkedProfile ? "EXISTS" : "NOT FOUND"}`);
    ...
    console.log(`   🎮 Game Rewards: ${summary.gameRewards.balanced + summary.gameRewards.unbalanced} USDT`);
