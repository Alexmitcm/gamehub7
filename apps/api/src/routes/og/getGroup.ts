...
const getGroup = async (ctx: Context) => {
  ...
  if (!address || typeof address !== "string") {
    return ctx.html(
      ...
      <html>
        <head>
          <title>Group Not Found - Hey</title>
          <meta name="description" content="The requested group could not be found." />
        </head>
        <body>
  ...
  return generateOg({
    buildHtml: (group: GroupFragment, escapedJsonLd) => {
      try {
        ...
        console.error("Error building OG HTML for group:", error);
        return html`
          <html>
            <head>
              <title>Group - Hey</title>
              <meta name="description" content="View this group on Hey" />
            </head>
            <body>
              <h1>Group</h1>
              <p>View this group on Hey</p>
            </body>
          </html>
        `;
      }
    ...
    buildJsonLd: (group: GroupFragment) => {
      try {
        ...
        console.error("Error building JSON-LD for group:", error);
        return {
          "@context": "https://schema.org",
          "@type": "Organization",
          name: "Group on Hey",
    ...
    extractData: (data) => {
      try {
        return data.group as GroupFragment | null;
      } catch (error) {
