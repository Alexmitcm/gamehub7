...
async function runTests() {
  ...
  try {
    ...
    const invalidInputResponse = await makeRequest(
      "POST",
      "/premium/link-profile",
      {
        "X-Access-Token": testJwt
    ...
    console.log(
      `   Response: ${JSON.stringify(invalidInputResponse.data, null, 2)}`
    );
    ...
    const unlinkResponse = await makeRequest(
      "DELETE",
      "/premium/unlink-profile",
      {
        "X-Access-Token": testJwt
      }
    );
    ...
    console.log(
      `   Response: ${JSON.stringify(profilesResponse.data, null, 2)}`
    );
    ...
    const walletStatusResponse = await makeRequest(
      "GET",
    ...
    console.log(
      `   Response: ${JSON.stringify(walletStatusResponse.data, null, 2)}`
    );
