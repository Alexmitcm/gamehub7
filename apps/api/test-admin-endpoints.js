...
async function testAdminEndpoints() {
  ...
  try {
    ...
    if (statsResponse.ok) {
      ...
      console.log(`   - Admin users: ${statsData.data?.adminUsers?.total || 'N/A'}`);
    } else {
    ...
    if (actionsResponse.ok) {
      ...
      console.log(`   - Actions count: ${actionsData.data?.actions?.length || 0}`);
    } else {
    ...
    const adminUserResponse = await fetch(`${API_BASE}/admin/admin-user?walletAddress=0x1234567890abcdef1234567890abcdef12345678`);
