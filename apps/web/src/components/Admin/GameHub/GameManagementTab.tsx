...
const GameManagementTab = () => {
  ...
  const fetchGames = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://gamehub7-production.up.railway.app/games/manage");
      if (response.ok) {
  ...
  const handleDeleteGame = async (gameId: string) => {
    ...
    try {
      const response = await fetch(`https://gamehub7-production.up.railway.app/games/manage/${gameId}`, {
        method: "DELETE"
      });
